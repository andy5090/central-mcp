...
def _collect_surfaces(node: dict[str, Any]) -> list[dict[str, Any]]:
    """Walk a layout subtree and return every terminal surface leaf."""
    if "surfaces" in node:
        return list(node["surfaces"])
...
class TestBuildLayoutJson:
    def test_empty_registry_no_orchestrator_single_placeholder(
        ...
        assert out["name"] == cmux.SESSION
        assert out["cwd"] == "."
        layout = out["layout"]
        assert layout == {"surfaces": [{"type": "terminal", "focus": True}]}
    ...
    def test_orchestrator_only_is_single_pane_leaf(
        ...
        layout = out["layout"]
        assert "surfaces" in layout
        assert "direction" not in layout
    ...
    def test_orchestrator_plus_one_project_splits_horizontally(
        ...
        orch = OrchestratorPane(command="claude", cwd=str(tmp_path), label="stub")
        out = cmux.build_layout_json(orch, registry.load_registry())
        layout = out["layout"]
        assert layout["direction"] == "horizontal"
        assert layout["split"] == 0.5
        # Left: orchestrator pane.
        first_surfaces = _collect_surfaces(layout["first"])
        assert len(first_surfaces) == 1
        assert first_surfaces[0]["name"] == "Central MCP Orchestrator"
        assert first_surfaces[0]["focus"] is True
        # Right: the single project pane (one surface, readonly-wrapped).
        right_surfaces = _collect_surfaces(layout["second"])
    ...
    def test_projects_only_first_surface_gets_focus(
        ...
        out = cmux.build_layout_json(None, registry.load_registry())
        surfaces = _collect_surfaces(out["layout"])
...
class TestEnsureWorkspace:
    ...
    def test_creates_workspace_when_missing(
        ...
        assert layout["name"] == cmux.SESSION
        assert layout["layout"]["direction"] == "horizontal"
