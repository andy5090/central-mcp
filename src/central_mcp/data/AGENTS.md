...
- "Send this to X: <prompt>" → prefer `dispatch_background(X, "<prompt>")` so the
  ...
  `dispatch_query` only if the user says "wait for this."
...
- **Prefer `dispatch_background`** for most dispatches. It returns immediately and lets the conversation continue. Poll with `check_dispatch`, cancel with `cancel_dispatch`, enumerate with `list_dispatches`.
- Use `dispatch_query` (synchronous, blocking) only when the user explicitly wants to wait for the result before moving on.
