...
이 설계가 중요한 이유는 실제 사용이 "책상 앞 6-pane tmux 대시보드" 가 아니기 때문입니다. 상당 부분의 사용이 **이동 환경·모바일** 이며, 사용자가 폰/태블릿에서 SSH 또는 원격 접속으로 머신에 붙어 오케스트레이터와 대화하며 여러 프로젝트를 한꺼번에 굴립니다. 그 맥락에서 멀티플렉서 그리드는 오히려 방해 — 화면을 잡아먹고, 작은 터미널과 싸우고, 내용의 대부분은 어차피 오케스트레이터가 요약해 주는 것의 더 noisy한 버전입니다.
