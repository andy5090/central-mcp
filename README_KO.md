...
모든 디스패치는 프로젝트 cwd에서 새 서브프로세스를 띄우는 방식(예: `claude -p "..." --continue`). 장기 프로세��� 관리 불필요, 화면 스크래핑 없음, 크리티컬 패스에 tmux 의존 없음.
...
3. **디스패치 라우터 프리앰블.** 오케스트레이터는 순수 라우터로 동작 — 프로젝트��� 파싱, `dispatch` 호출, 다음 요청. LLM 추론 지연 ~1-2초.
...
- *"내 프로젝트 목록은?"*
- *"my-app에 보내줘: auth 모듈�� 에러 핸들링 추가해."*
...
서브에이전�� 모델은 독립적 — 각 `dispatch`는 프로젝트 에이전트의 기본 모델로 자체 프로세스를 생성합니다.
...
`central-mcp up`은 tmux 세션 `central`에 프로젝��당 인터랙티브 pane을 하나씩 만듭니다. `Ctrl+b n` / `Ctrl+b <숫자>`로 pane 전환. **순수 시각적** — MCP 디스패치 경로는 이 pane을 읽거나 쓰지 않습니다. `central-mcp down`으로 종료해도 오케스트레이터에 영향 없음.
