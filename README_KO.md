...
### 왜 *선택적* 인가 (그리고 기본이 "pane 없이 기동" 인 이유)

central-mcp는 **오케스트레이터 자체가 사용자의 인터페이스**가 되도록 설계되었습니다 — 터미널 pane의 벽이 아니라. `dispatch` / `check_dispatch` / `list_dispatches` / `orchestration_history` 모든 MCP 호출이 구조화된 요약(출력, exit code, duration, 시도 내역, 폴백 여부, 프로젝트별 집계)을 반환하고, 오케스트레이터가 그걸 읽어서 자연어로 상황을 보고합니다. "리팩토링 어떻게 됐어?"라고 물으면 오케스트레이터가 답합니다 — 스크롤되는 stdout을 눈으로 쫓을 필요가 없습니다.

이 설계가 중요한 이유는 central-mcp가 **언제·어디서든 프로젝트를 진행할 수 있어야** 한다는 방향을 추구하기 때문입니다 — 책상에서 멀리 떨어져 폰·태블릿으로 SSH/원격 접속만 가능한 순간도 포함해서. 이게 실제 사용의 대다수라서가 아니라, 허브가 멀티-pane 대시보드 앞에 앉아 있을 때만 쓸 수 있다면 그 외의 순간에 진도를 못 내기 때문. 그래서 오케스트레이터 혼자서도 충분하도록 설계되어 있습니다 — 자연어로 지시하면 플릿을 돌리고 구조화된 요약을 돌려줍니다. 그 위에 멀티플렉서 그리드를 얹는 것은 여건이 될 때 이득이지만 전제조건은 아닙니다.

그래서 **실시간 모니터링이 명확히 필요한 경우가 아니라면** (토큰 스트리밍을 보고 싶다, 멈춘 dispatch를 실시간으로 잡고 싶다, 긴 마이그레이션 출력을 tail하고 싶다 등) 관찰 레이어를 쓸 필요가 없습니다. 허브는 이 없이도 정상 동작하고, 사후 리뷰는 `dispatch_history` / `orchestration_history` 로 충분합니다.

진짜로 라이브 뷰가 이득인 순간(로컬 데스크톱, 멈춘 에이전트 디버깅, 세션 중 플릿 screen-share 등)에만 켜세요.
