...
오케스트레이터는 기동시 `~/.central-mcp/AGENTS.md` 를 읽는데 거기에 터미널 크기에 따라 분기되는 레시피가 들어있어, Bash 툴로 `cmux new-split`, `cmux send`, `cmux send-key` 를 프로젝트마다 연결합니다. 레이아웃 동작은 tmux / zellij 와 동일한 모양 — cmux 윈도우가 넓으면 같은 워크스페이스 안에 오케스트레이터 열 + 프로젝트 grid (`main-vertical` 과 같은 구조), 좁으면 관찰 레이어를 별도 `central-mcp watch` 워크스페이스로 분리해서 cmux 사이드바에서 탭 전환.
