...
오케스트레이터는 기동시 `~/.central-mcp/AGENTS.md` 를 읽는데 거기에 터미널 크기에 따라 분기되는 레시피가 들어있어, Bash 툴로 `cmux new-split`, `cmux send`, `cmux send-key` 를 프로젝트마다 연결합니다. 워크스페이스 네이밍은 tmux / zellij 의 윈도우 컨벤션을 그대로 따름 — 오케스트레이터의 원래 워크스페이스는 `cmcp-hub` 로 리네임되고, 관찰 pane 들은 `cmcp-watch-1`, `cmcp-watch-2`, … 라는 별도 워크스페이스에 배치됩니다 (터미널 크기로 grid chunk 개수 결정). cmux 사이드바에서 탭 전환.
