...
### Added
- **`cmcp tui --agent codex --experimental` — TUI Phase B.** Codex joins claude inside the embedded Textual UI: same chrome (header / sidebar / footer), same dispatch watcher, same `Ctrl+Q` chrome-only quit. The CSI leak filter and whitespace-emphasis stripping carry over so codex's banner / table borders / tool-result separators render cleanly. `--agent` is now a constrained choice (`claude` / `codex`); gemini / opencode are on the roadmap (Phase C).
- **PtyTerminal as a dispatch event writer.** When constructed with `project=` / `agent=` / `cwd=`, the widget's new `submit_prompt(text)` method writes the prompt bytes into the master fd *and* records `start` in `dispatches.db` + `dispatch.jsonl` (`mode="pty"`). A screen-stability watcher hashes (cursor + bottom 6 rows of pyte buffer) every 0.5s; once 3 consecutive ticks match, the dispatch flips to `complete` and `DispatchWatcher` picks it up on its next mtime check — same surfaces the MCP `dispatch()` tool writes to, no MCP round-trip. Building block for upcoming per-project REPL panes; the Phase B TUI itself still hosts an unbound orchestrator REPL so nothing observable changes for users on this release.
- **PTY session registry + `dispatch()` guard.** New `pty_sessions` module manages `~/.central-mcp/pty-sessions/<project>.json` lifecycle — written by the TUI's PtyTerminal on spawn, removed on unmount, with stale-PID sweep on read so a TUI crash never leaves a project permanently locked. `dispatch()` consults the registry and returns `{ok: false, mode: "pty"}` for projects with a live PTY pane, so background fan-out can't inject prompts into a pane the user is supervising. Public read API: `pty_sessions.is_active(project)` / `get(project)` / `list_active()`. First piece of the `Live agent panes` roadmap track.
- **Hermes Agent (Nous Research) joins the orchestrator + dispatch lineup.** New `_Hermes` adapter wraps `hermes -z PROMPT` for one-shot dispatch with `--continue` / `--resume <id>` for session resumption and `--yolo --accept-hooks` for bypass mode. Registered in `agents.AGENTS` with `can_dispatch / can_orchestrate / mcp_installable = True`. `cmcp install hermes` writes central-mcp into `~/.hermes/config.yaml` under `mcp_servers.central` using ruamel.yaml so the user's comments and formatting round-trip. The CLI's `install` subcommand now derives its `client` choices from `agents.SUPPORTED_CLIENTS` instead of hardcoding the list, so future installer additions propagate without a second edit. Combined with Hermes's existing `hermes mcp add` and `hermes mcp serve`, this enables bidirectional integration: cmcp can dispatch to Hermes as a project agent OR run Hermes as the orchestrator (`cmcp run --agent hermes`), and Hermes natively sees cmcp's `dispatch` / `list_projects` / `check_dispatch` tools as its own.
- **Hermes usage in the quota HUD.** New `central_mcp.quota.hermes` fetcher rolls `~/.hermes/state.db` sessions table into hour / day / week windows (input / output / cache_read / cache_write / reasoning tokens, plus `actual_cost_usd`). The snapshot exposes hermes alongside claude / codex / gemini under a new `mode: "local_ledger"` shape — different units (absolute spend in window) than claude / codex's percent-of-cap, but useful where it shows up: orchestrators can now answer "what did Hermes burn through today?" without a separate tool. `agents.AGENTS["hermes"].has_quota_api` flips to True. Reads the db with `mode=ro` URI so a running `hermes` process holding the write lock never stalls our snapshot, and degrades cleanly to `mode: "not_installed"` when the file is absent. `has_session_reader` stays off until a cwd-discovery spike (Hermes's sessions table doesn't track working directory, so mapping a project's cwd → its sessions needs a different signal than claude / codex provide).

### Notes
- `cmd_tui`'s agent gate moved from a hardcoded `agent != "claude"` to a tuple of supported agents, mirrored by `tui.app.SUPPORTED_AGENTS` so the TUI app and the CLI gate can't drift.
- PTY-mode dispatches do not capture stdout into `dispatches.output` and do not emit per-chunk `output` events — the live screen is in the pane itself, and structured chunk extraction from raw ANSI is deferred. Tokens are unavailable too; both gaps are acceptable for the building-block release.
- Tests: `test_unsupported_agent_rejected` parametrized over four still-unsupported names so adding a new agent surfaces the gate change in one place; new `test_supported_agents_pass_gate` walks both supported names through `cmd_tui` with a stubbed `run_tui` so the contract is locked. `test_missing_tui_extras_actionable` now also clears the parent-package `app` attribute — earlier tests in the suite import `central_mcp.tui.app` and pin it on the package namespace, which would otherwise short-circuit the sys.modules-based ImportError simulation. New `test_submit_prompt_*` / `test_screen_hash_*` / `test_mark_complete_*` lock submit/track/finalize behaviour against the real `dispatches.db` + `dispatch.jsonl` writers. New `tests/test_pty_sessions.py` (9 tests) covers register / unregister / stale-PID sweep / corrupt-entry sweep / dispatch-guard lifecycle, including end-to-end "block then unregister then dispatch succeeds" via the stub adapter. New `TestHermes` class in `test_adapters.py` (6 tests) locks the argv shape across resume / no-resume / bypass / session-id paths and asserts the capability table flags. New hermes-install tests in `test_install.py` (4 tests) cover the YAML round-trip, idempotence, missing-config refusal, and dry-run path. New hermes quota tests in `test_quota.py` (8 tests) build a synthetic `state.db` per case to lock window-cutoff math, NULL-cost handling, missing-db, corrupt-db, normalize layer, and snapshot inclusion. 600/600 passing.
