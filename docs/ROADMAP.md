...
The python-sdk **v2 beta already implements the full RC** (pluggable extensions API included), with stable v2 targeted for the spec's publication date — so this track starts now, against the beta, rather than waiting for the final spec. Phasing:

🚧 **Phase 1 (now) — task-model groundwork, no SDK dependency.** Everything spec-shaped that lives in our own code: align `dispatches.db` status values with the Tasks lifecycle vocabulary (`working` / `input_required` / `completed` / `failed` / `cancelled`) behind a mapping table, isolate a `tasks_adapter` module that translates a dispatch row ↔ a spec-conformant task object, and audit the server for reliance on the deprecating trio (Roots / Sampling / Logging — 12-month window). All of this is testable today against the RC schemas and none of it changes existing tool behavior.

📋 **Phase 2 (now, behind a flag) — wire the extension on the v2 beta.** Add an optional `mcp>=2.0.0b` path (extra or version gate) that advertises the Tasks extension and serves `tasks/get` / `tasks/cancel` from the same `dispatches.db` rows via the Phase-1 adapter. `check_dispatch` / `cancel_dispatch` stay as-is indefinitely — the extension is an additional wire shape over the same state, not a replacement. Clients that speak Tasks (Claude Code and others are adopting it) then get dispatch polling, progress, and cancellation natively, including built-in "keep polling until done" loops we currently have to prompt-engineer into codex / gemini.

📋 **Phase 3 (on stable v2) — flip the default + conformance pass.** Pin stable `mcp>=2`, drop the flag, and do the mechanical stateless-core conformance sweep (header validation, `ttlMs` / `cacheScope` on list responses — largely SDK-handled). central-mcp is already stateless between requests by design (a load-bearing invariant), so no architectural work is expected here; Phases 1–2 will have absorbed the real changes before the spec even finalizes.
