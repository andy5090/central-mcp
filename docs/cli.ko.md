...
임베디드 Textual 앱: orchestrator 에이전트가 메인 페인의 PTY 안에서 돌고, 사이드바(토큰 사용량 HUD + 활성 dispatch + 최근 완료)가 둘러쌈. `dispatches.db`를 직접 polling해서 dispatch 완료에 즉시 반응 — MCP 클라이언트의 알림 forward에 의존 안 함. 옵션 의존성 `[tui]` 필요: `pip install 'central-mcp[tui]'`. `--experimental` 플래그 강제 (1.0에서 졸업 시점까지). 0.14.0(Phase C)부터 4개 orchestrator 전부 지원.
