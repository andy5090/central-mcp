...
본질 섹션이 central-mcp가 *무엇을 위한 것인지*를 말한다면, 이 섹션은 *어디서 만나는지*를 말합니다 — 순위를 매겨서요. 투자는 순위를 따라가기 때문입니다. 명시적 순서가 없으면 가장 덜 쓰이는 표면에 가장 많이 투자하기 쉽습니다. 이전 로드맵이 정확히 그랬습니다 — TUI 안정화 단독이 1.0을 정의했으니까요.

핵심 관찰: **전용 orchestrator REPL은 찾아가야 하는 목적지이고, 찾아가야 만나는 PM은 잊힙니다.** 따라서 주력 표면은 이미 열려 있는 에이전트 세션입니다.
...
여기서 따라 나오는 결론 둘을 명시합니다:

📋 **`cmcp run`은 1.0에서 fallback으로 강등.** 3단의 의도된 표면은 TUI입니다. `run`은 extra 설치 없는 런처(`[tui]` 불필요)이자 textual을 못 띄우는 터미널의 탈출구로 남습니다. 제거가 아니라 강등입니다.
