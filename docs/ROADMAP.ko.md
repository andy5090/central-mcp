...
💭 **Lazy daemon — TUI 트랙 이후로 결정 보류.** daemon이 있으면 `dispatches.db` watching, OS 알림, 세션 스캐너를 한 곳에 모을 수 있지만, TUI(0.12.0+) 자체가 long-running 프로세스라 그 책임을 이미 흡수합니다. TUI 1.0 출시 후 두 가지 결말 가능:
  - (a) **daemon 불필요.** TUI가 자기 완결, TUI를 안 띄운 사용자는 지금처럼 폴링 / piggyback으로 충분.
  - (b) **TUI 없는 환경용 daemon.** "TUI 안 띄우고도 완료 알림은 받고 싶다"는 분명한 시그널이 생기면, 슬림한 daemon (dispatches.db watcher + OS 알림만, MCP proxy 없음)이 1.x feature로 합류.

  cold-start 절감 (~150ms) 단독으로는 복잡도를 정당화 못 합니다 — cross-process state는 0.10.9의 `dispatches.db`로 이미 풀렸음.

💭 **MCP push notifications — 여전히 막힘, 시급성 낮아짐.** 완료된 dispatch에 대한 server-initiated `notifications/resources/updated` 이벤트는 여전히 어떤 MCP 클라이언트가 LLM 턴으로 surface하느냐에 묶여 있습니다. TUI는 이걸 우회 (TUI 내부의 daemon-스타일 watcher가 MCP 측 협조 없이 동작)하므로, 업스트림이 막힌 채로 있어도 central-mcp 사용자는 `cmcp tui`로 알림 경로를 가질 수 있습니다. push notifications는 MCP 클라이언트가 forward를 약속하는 시점에 합류 — 비-TUI 사용자에겐 여전히 정답이지만, 더 이상 *유일한* 길은 아닙니다.
