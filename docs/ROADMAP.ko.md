...
- **별도 daemon 프로세스.** TUI(`cmcp tui`, 0.12.0+) 자체가 long-running watcher입니다 — asyncio task가 LLM 턴과 무관하게 `dispatches.db`를 tail하고, 완료를 사이드바로 surface하고, (옵션) 에이전트 PTY에 hint를 주입해서 사용자가 trigger 안 해도 LLM이 깨어나서 보고합니다. 그게 daemon이 풀어줬을 motivation 전부입니다. cross-process state는 0.10.9의 `dispatches.db`로 이미 풀렸음. 만약 1.0 출시 후 "TUI 안 띄우고도 알림 받고 싶다"는 신호가 강하게 오면 슬림한 watcher daemon이 1.x add-on으로 합류할 수 있지만, 1.0 가는 길에는 없습니다.
