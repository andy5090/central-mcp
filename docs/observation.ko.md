---
description: dispatch 흐름을 멀티 페인으로 라이브로. cmux의 에이전트 주도 페인 철학과 central-mcp의 stateless / 로그 기반 모델이 어떻게 맞아떨어지는지.
---
...
관찰 모드는 그 흐름들을 **라이브로 보게 해주는 화면**입니다. 지금 따라가는 프로젝트마다 한 페인, orchestrator는 옆에.

> **페인은 지도가 아니라 현미경입니다.** 프로젝트 하나를 요약 없이 가까이 보여줌으로써 화면을 차지할 자격을 얻습니다. *전체가 어디 있나?* 라는 포트폴리오 질문은 [`cmcp pulse`](cli.md)와 TUI 사이드바가 답합니다 — 이쪽은 dispatch 로그뿐 아니라 git도 읽습니다. 그래서 0.16.0부터 백엔드들은 등록된 전 프로젝트를 기본으로 타일링하지 않습니다. 한 창에 다 들어가면 전부 뜨고, 넘치면 들어갈 수 있는 만큼 가장 최근에 활동한 것들이 뜨며 나머지는 이름과 함께 안내됩니다. `--projects a,b,c`로 직접 고르고, `--all-projects`로 전체 타일링을 복원합니다.
>
> 이 구분은 반대 방향으로도 중요합니다. pulse 브리핑은 **LLM이 요약한 것**이고 watch 페인은 원본 출력입니다. 에이전트가 "테스트 통과"라고 했는데 실제 테스트 실행을 눈으로 봐야 할 때, 그게 페인이 존재하는 이유입니다.
...
- **상주 데몬 없음.** central-mcp는 프로젝트마다 `dispatch.jsonl`만 남깁니다. 무엇을 어디에 그릴지는 에이전트가 결정합니다.
...
결과: 사용자는 진짜 macOS 앱 안에 있고, 한쪽 페인에서는 에이전트(Claude Code / Codex / Gemini / opencode / Hermes Agent / gajae-code)와 대화하고 있고, 한 문장 뒤에 그 주변으로 프로젝트 페인 그리드가 깔립니다. 각 페인은 자기 프로젝트의 dispatch 출력을 라이브로 보여줍니다. tmux 설정도, 외울 키 바인딩도, 터미널 에뮬레이터 오버헤드도 없습니다.
...
위에 해당하지 않으면 관찰 모드는 건너뛰셔도 됩니다. central-mcp는 어느 쪽이든 똑같이 동작합니다.
