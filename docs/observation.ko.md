...
결과: 사용자는 진짜 macOS 앱 안에 있고, 한쪽 페인에서는 에이전트(Claude Code / Codex / Gemini / opencode / Hermes Agent)와 대화하고 있고, 한 문장 뒤에 그 주변으로 프로젝트 페인 그리드가 깔립니다. 각 페인은 자기 프로젝트의 dispatch 출력을 라이브로 보여줍니다. tmux 설정도, 외울 키 바인딩도, 터미널 에뮬레이터 오버헤드도 없습니다.
